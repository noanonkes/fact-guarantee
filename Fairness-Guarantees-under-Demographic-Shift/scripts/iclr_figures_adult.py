...
if __name__ == '__main__':
    with ArgumentSweeper() as parser:
        ...
        parser.add_argument('--mlp',  action='store_true',
                            help='Generates results for mlp experiments.')
    ...
    if args.mlp:
        di_path = f'results/results_mlp_experiments/iclr_adult_{mode}_ds_rl_di_0/iclr_adult_{mode}_ds_rl_di.h5'
        dp_path = f'results/results_mlp_experiments/iclr_adult_{mode}_ds_rl_dp_0/iclr_adult_{mode}_ds_rl_dp.h5'
    else:
        di_path = f'results/results_original_experiments/iclr_adult_{mode}_ds_rl_di_0/iclr_adult_{mode}_ds_rl_di.h5'
        dp_path = f'results/results_original_experiments/iclr_adult_{mode}_ds_rl_dp_0/iclr_adult_{mode}_ds_rl_dp.h5'
...
def plotting_NSF_acc(path):
    ...
    if not (os.path.exists(path)):
        ...
        if save_figs:
            if args.mlp:
                save(fig, f'{path[-28:-3]}_MLP_P(NSF).{fmt}', dpi=dpi)
            else:
                save(fig, f'{path[-28:-3]}_P(NSF).{fmt}', dpi=dpi)
        else:
            fig.show()
...
def plotting_FR(path):
    ...
    if not (os.path.exists(path)):
        ...
        fig, [ax_ofr, ax_dfr] = plt.subplots(ncols=2, figsize=(10, 5))
        ...
        for mn, c, fr, se in zip(mnames[::-1], colors[::-1], (ofrates.T)[::-1], (ofrates_se.T)[::-1]):
            ...
            ax_ofr.fill_between(nvals, (fr+se)*100, (fr-se)
                                * 100, color=c, linewidth=0, alpha=0.25)
        ...
        for mn, c, fr, se in zip(mnames[::-1], colors[::-1], (dfrates.T)[::-1], (dfrates_se.T)[::-1]):
            line = ax_dfr.plot(nvals, fr*100, c=c,
                               ls=get_ls(mn), lw=get_lw(mn))[0]
            ax_dfr.fill_between(nvals, (fr+se)*100, (fr-se)
                                * 100, color=c, linewidth=0, alpha=0.25)
            pmn = pprint_map[mn]
            if not (pmn in added):
                added.append(pmn)
        ...
        # Finalize the figure and display/save
        for ax in [ax_ofr, ax_dfr]:
            ax.spines['right'].set_visible(False)
            ax.spines['top'].set_visible(False)

        if save_figs:
            if args.mlp:
                save(fig, f'{path[-28:-3]}_MLP_failure_rates.{fmt}', dpi=dpi)
            else:
                save(fig, f'{path[-28:-3]}_failure_rates.{fmt}', dpi=dpi)
        else:
            fig.show()
