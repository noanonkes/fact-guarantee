...
def _get_fairlearn(dataset, mp):
	...
	try:
		results, hs = f_l.expgrad(Xt, Tt, Yt, base_model, cons=cons, eps=mp['fl_e'])
...
def _get_fair_constraints(dataset, mp):
	...
	sensitive_attrs_to_cov_thresh = {'S':0.1, 
									 'R':0.01}
	w = f_c.train_model(X, Y, x_control, f_c._logistic_loss, apply_fairness_constraints, apply_accuracy_constraint, sep_constraint, sensitive_attrs, sensitive_attrs_to_cov_thresh, gamma)
	def predictf(_X):
